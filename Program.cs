...
namespace TransactionDemoConsole
{
    partial class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                using (DocumentClient client = new DocumentClient(new Uri(EndpointUrl), AuthorizationKey, ConnectionPolicy))
                {
                    ...
                    /* Alex <=> Max */
                    RunExchangeTransaction(client, "Alex","Max").Wait();

                    ReviewPlayers(client).Wait();
                    /* Scott <=X=> Alex */
                    RunExchangeTransaction(client, "Scott", "Alex").Wait();
        ...
        private static async Task RunExchangeTransaction(DocumentClient client, string player1, string player2)
        {
            try
            {
